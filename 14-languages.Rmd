...
Next we show an example of an alternative Python engine[^1] named `py` and implemented by simply calling the `python` command via `system2()`:

[^1]: In practice, you should use the built-in `python` engine instead, which is based on the **reticulate** package and supports Python code chunks much better.
