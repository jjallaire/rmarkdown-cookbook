...
For example, below is a trick that turns hyperlinks on text into footnotes. This trick is useful when the PDF output document is printed on paper, because readers will not be able to click the links (generated from `\href{URL}{text}`) on paper but can see the URLs in footnotes. This trick displays both the text and URL.
...
For this particular trick, you do not really have to implement it by yourself, but can simply set the YAML option `links-as-notes` to `true` because it is a built-in feature of Pandoc's default LaTeX template (see Section \@ref(latex-variables)).
...
There is an extensive community for LaTeX, and there are over 4,000 packages available through the [Comprehensive TeX Archive Network](https://ctan.org) (CTAN). As some examples of LaTeX packages you could consider using within your report:
...
In general, we do not recommend that users force LaTeX to stop floating figures. This solution was included in this book by popular demand,[^latex-2] but there could be some serious side-effects when LaTeX is unable to float figures.

[^latex-2]: The related Stack Overflow question has been viewed for over 45,000 times: <https://stackoverflow.com/q/16626462/559676>.
